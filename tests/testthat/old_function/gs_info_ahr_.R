...
gs_info_ahr_ <- function(enrollRates=tibble::tibble(Stratum="All",
                                                   duration=c(2,2,10),
                                                   rate=c(3,6,9)),
  ...
  avehr <- NULL
  if(!is.null(analysisTimes)){
    avehr <- AHR_(enrollRates = enrollRates, failRates = failRates, ratio = ratio,
                            totalDuration = analysisTimes)
    for(i in seq_along(events)){
      if (avehr$Events[i] < events[i]){
        avehr[i,] <- tEvents_(enrollRates = enrollRates, failRates = failRates, ratio = ratio,
    ...
    for(i in seq_along(events)){
      avehr <- rbind(avehr,
                     tEvents_(enrollRates = enrollRates, failRates = failRates, ratio = ratio,
                                        targetEvents = events[i]))
    }
  }
